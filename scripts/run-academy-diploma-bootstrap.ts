/**
 * Apply academy_diploma_purchases migration (0009).
 * Usage: npm run db:bootstrap:academy-diploma
 */
...
async function runViaManagementApi(sql: string): Promise<void> {
  ...
  console.log("Management API: academy_diploma_purchases migration applied");
}
...
async function runViaPostgres(sql: string): Promise<void> {
  ...
  try {
    ...
    console.log("Postgres: academy_diploma_purchases migration applied");
  } finally {
...
async function main() {
  ...
  const sql = readFileSync(
    join(process.cwd(), "supabase/migrations/0009_academy_diploma_purchases.sql"),
    "utf8"
  );
