/**
 * Apply academy_reminder_prefs migration to Supabase.
 *
 * Usage:
 *   npm run db:bootstrap:academy-reminders
 */
...
async function runViaManagementApi(sql: string): Promise<void> {
  ...
  console.log("Management API: academy_reminder_prefs migration applied");
}
...
async function runViaPostgres(sql: string): Promise<void> {
  ...
  try {
    ...
    console.log("Postgres: academy_reminder_prefs migration applied");
  } finally {
...
async function main() {
  ...
  const migrationPath = join(
    process.cwd(),
    "supabase/migrations/0007_academy_reminder_prefs.sql"
  );
