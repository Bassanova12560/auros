/**
 * Apply green_label_applications migration (0011).
 * Usage: npm run db:bootstrap:green-label
 */
...
async function runViaManagementApi(sql: string): Promise<void> {
  ...
  console.log("Management API: green_label_applications migration applied");
}
...
async function runViaPostgres(sql: string): Promise<void> {
  ...
  try {
    ...
    console.log("Postgres: green_label_applications migration applied");
  } finally {
...
async function main() {
  ...
  const sql = readFileSync(
    join(process.cwd(), "supabase/migrations/0011_green_label_applications.sql"),
    "utf8"
  );
