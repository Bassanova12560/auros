/**
 * Apply academy_cert_registry migration (0010).
 * Usage: npm run db:bootstrap:academy-registry
 */
...
async function runViaManagementApi(sql: string): Promise<void> {
  ...
  console.log("Management API: academy_cert_registry migration applied");
}
...
async function runViaPostgres(sql: string): Promise<void> {
  ...
  try {
    ...
    console.log("Postgres: academy_cert_registry migration applied");
  } finally {
...
async function main() {
  ...
  const sql = readFileSync(
    join(process.cwd(), "supabase/migrations/0010_academy_cert_registry.sql"),
    "utf8"
  );
