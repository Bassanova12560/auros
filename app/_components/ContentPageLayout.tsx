...
type Props = {
  ...
  /** Optional product line on the brand lockup */
  product?: string;
  /** Hide lockup only for rare full-bleed heroes that already brand themselves */
  hideBrand?: boolean;
};

/**
 * Standard content page frame inside FocusPageShell.
 * Always signals AUROS before the page title.
 */
export function ContentPageLayout({
  ...
  return (
    <article className="auros-page-article">
      {!hideBrand ? (
        <div className="mb-8 md:mb-10">
          <AurosBrandLockup product={product} size="md" />
    ...
    <div className="divide-y divide-white/[0.06] border-y border-white/[0.06]">
      {items.map((item) => (
        <details key={item.question} className="group">
          ...
          <div className="pb-5 pl-7 text-sm leading-relaxed text-white/55">
            {item.answer}
          </div>
        </details>
      ))}
