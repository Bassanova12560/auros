...
/** WattCoin lab: mint WATT 1:1 with akWh collateral, or redeem. */
  ...
  try {
    const snap =
      parsed.data.action === "mint"
        ? await mintWatt({ accountId: parsed.data.accountId, amount: parsed.data.amount })
        : await redeemWatt({ accountId: parsed.data.accountId, amount: parsed.data.amount });
