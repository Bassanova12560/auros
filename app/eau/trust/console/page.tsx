...
import { WETS_WELCOME_PATH } from "@/lib/vertical-welcome/config";
import { listWetsTrustScores } from "@/lib/wets/store";

import { WetsGradeBadge, WetsNav } from "../_components/WetsUi";

export const metadata: Metadata = {
  title: "Console WETS | AUROS",
  description: "Console privée Water/Energy Trust Score.",
...
export default async function WetsConsolePage({
  ...
  if (!userId) {
    return (
      <FocusPageShell path={WETS_CONSOLE_ROUTE} width="3xl">
        <ContentPageLayout
          product="Eau · Trust"
          eyebrow="Console WETS"
          title="Connexion requise"
          intro="La console projet est réservée aux comptes connectés."
        >
          <div className="flex flex-wrap gap-3">
            <PrimaryButton href="/sign-in">Se connecter</PrimaryButton>
            <PrimaryButton href={WETS_WELCOME_PATH} variant="ghost">
              Page d&apos;accueil WETS
            </PrimaryButton>
          </div>
          <p className="mt-8 text-sm text-white/50">{WETS_DISCLAIMER}</p>
        </ContentPageLayout>
      </FocusPageShell>
    );
  }
  ...
  return (
    <FocusPageShell path={WETS_CONSOLE_ROUTE} width="3xl">
      <ContentPageLayout
        product="Eau · Trust"
        eyebrow="Console · Water/Energy Trust Score"
      ...
      >
        ...
        {filtered.length === 0 ? (
          <p className="text-sm text-white/50">
            Aucun projet. Commencez par un actif eau/énergie pour valider la
            grille.
          </p>
        ) : (
